...
\begin{document}
	...
	\begin{table}[h!]
		...
		\begin{subtable}{\textwidth}
			...
			\begin{tabular}{l | *{3}{|r}}
				...
				Training \& test loops       & 85\%             &            10\% &             5\% \\ \hline
				Coding visualisations        & 65\%             &           20\% &            15\% \\ \hline
				Report writing               & 60\%             &           20\% &            20\% \\ \hline
	...
	(one hot encoded $\beta_9$ to $\beta_{14}$) to the refractive index (\texttt{RI}). The elements \texttt{Ca}, \texttt{Na} and \texttt{Ba} exhibit the largest positive coefficients, implying that increasing these oxides raises the estimated refractive index in this model.
	Conversely, \texttt{Si} and \texttt{Al} have negative coefficients,
	...
	suggesting that this type slightly decreases the predicted \texttt{RI}. In this model, while the categorical glass types are included, the chemical composition dominates the predictions of \texttt{RI}.
	...
	We observe that the optimal $\hat{h}=120$ achieved the minimum average MSE of \num{0.025}, indicating that a moderately sized hidden layer performs best (we can say 120 is moderate out of 960 because it is substantial but not big in the context).
	...
	\begin{figure}
		...
		The optimal $h$ ($\hat{h}=120$) achieved a minimum average validation MSE of \num{0.025}.}
	...
	\begin{table}
		...
		All comparisons show statistically significant differences ($p<0.05$). It should be noted that ANN-RLR and ANN-Baseline comparisons look identical due to rounding. In reality, they \textit{do} slightly differ.}
...
These results clearly support the ranking \textbf{ANN $<$ Baseline $<$ RLR}, and Table~\ref{tab:reg_pairwise} quantifies the magnitude and statistical significance of the differences between models.
	...
	The two-level cross-validation results reveal that the Logistic Regression model
	achieves the best test errors overall, outperforming the Decision Tree in most cases, which can also be seen in Figure~\ref{fig:classification_test_errors_by_fold}
	and also substantially surpassing the baseline. The Logistic Regression hyperparameter $C_i^*$ vary
	widely (from \num{0.00428} to \SI{0.695}{}) across folds, indicating data-dependent regularization needs. Nonetheless, it consistently beats the baseline
	...
	\begin{table}
		...
		Mean differences $\hat{\mu}$ and standard deviations $\hat{\sigma}$ in test error $E^{\text{test}}$, $t$-statistics, and $p$-values are reported, as well as a 95\% confidence interval $[t_{0.05}, t_{0.95}]$.
		The last two comparisons show statistically significant differences ($p<0.05$), but DT-LR is \textit{not} significant ($p=0.37$).}
	...
	Generative AI (\textit{ChatGPT} by OpenAI and \textit{Claude} by Anthropic) was used as a support tool during the project. Its role was limited to assisting with code debugging, \LaTeX table formatting, identifying and fixing minor
